...
def main():
    ...
    total_tests += 1
    if run_command("python -c 'from manifoldbot import ManifoldReader, ManifoldWriter; print(\"Package imports work\")'", 
                   "Package Installation"):
        tests_passed += 1
    
    # Test 2: Basic initialization
    total_tests += 1
    if run_command("python -c 'from manifoldbot import ManifoldReader, ManifoldWriter; r=ManifoldReader(); w=ManifoldWriter(\"test\"); print(\"Initialization works\")'", 
                   "Basic Initialization"):
        tests_passed += 1
    
    # Test 3: Unit tests (no API calls)
    total_tests += 1
    if run_command("pytest tests/manifold/test_reader.py::TestManifoldReader -v --tb=short", 
                   "Unit Tests (Reader)"):
        tests_passed += 1
    
    # Test 4: Writer unit tests
    total_tests += 1
    if run_command("pytest tests/manifold/test_writer.py -v --tb=short", 
                   "Unit Tests (Writer)"):
        tests_passed += 1
    ...
    if manifold_key:
        total_tests += 1
        if run_command("pytest tests/manifold/test_reader_real.py -v --tb=short", 
                       "Real API Tests"):
            tests_passed += 1
    else:
