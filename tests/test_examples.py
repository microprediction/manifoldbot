...
class TestExamples(unittest.TestCase):
    ...
    def test_simple_trading_bot_example_imports(self):
        ...
        try:
            from manifoldbot.examples.simple_trading_bot import SimpleTradingBot, main
    ...
    def test_basic_writer_example_without_api_key(self):
        """Test that basic_writer example handles missing API key gracefully."""
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            try:
                ...
                self.fail(f"basic_writer example failed to handle missing API key: {e}")
    
    @patch('manifoldbot.examples.simple_trading_bot.ManifoldReader')
    @patch('manifoldbot.examples.simple_trading_bot.ManifoldWriter')
    def test_simple_trading_bot_example_runs(self, mock_writer_class, mock_reader_class):
        """Test that simple_trading_bot example can run without errors."""
        # Mock the reader
        mock_reader = MagicMock()
        mock_reader_class.return_value = mock_reader
        mock_reader.get_markets.return_value = [
